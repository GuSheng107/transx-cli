...
import { DEEPLX_URL_TEMPLATE } from "../src/constants.js";
...
test("API Key 可以保存、重置，URL 模板固定不可变", async () => {
  ...
  try {
    ...
    assert.equal(resolved.urlTemplate, DEEPLX_URL_TEMPLATE);

    await store.resetKey();
    const statusAfterKeyReset = await store.status({});
    assert.equal(statusAfterKeyReset.urlTemplate, DEEPLX_URL_TEMPLATE);
...
test("环境变量覆盖本地 Key，状态支持脱敏与完整 Key", async () => {
  ...
  try {
    ...
    assert.equal(status.urlTemplate, DEEPLX_URL_TEMPLATE);
